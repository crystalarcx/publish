...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("### 📝 其他功能")
        if st.button("📄 開啟空白加班單", type="secondary", help="開啟Google文件加班單"):
            st.markdown("""
            <script>
            window.open('https://docs.google.com/document/d/1T75rw_3hQtIaBTGMFxa09G93Atihf4h-883Kg1tqPpo/edit?usp=sharing', '_blank');
            </script>
            """, unsafe_allow_html=True)
            st.success("🔗 已在新分頁開啟空白加班單")
