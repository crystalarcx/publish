# 互動式員工班表加班時數統計系統 (Streamlit版) - 增強版
import pandas as pd
...
# 新增：記憶最後使用的版本資訊
if 'remembered_version_info' not in st.session_state:
    st.session_state.remembered_version_info = {
        'version_name': '',
        'main_sheet_url': '',
        'load_time': ''
    }
# 新增：記憶自定義假日設定
if 'saved_custom_holidays' not in st.session_state:
    st.session_state.saved_custom_holidays = {}
...
def calculate_overtime_summary(target_personnel, year, month, matching_columns):
    ...
    # 【新增功能 3】如果平日總時數超過46小時，刪除日期讓總時數等於或接近46小時
            ...
            try:
                ...
                if date_str in export_data:
                    ...
                    if is_weekend:
                        ...
                        # 【新增功能 4】假日當日時數如果小於3小時，則在原始時間字串前加2小時加班時數
                                ...
                                if '-' in first_time_part:
                                    ...
                                    try:
                                        ...
                                        if ':' in start_time:
                                            ...
                                            hour = int(start_time[:2]) if len(start_time) >= 2 else int(start_time)
                                            minute = 0
                                        
                                        # 計算前2小時的時間
                                        new_start_hour = hour - 2
                                        if new_start_hour < 0:
                                            new_start_hour += 24
                                        
                                        # 生成新的時間字串
                                        new_time_part = f"{new_start_hour:02d}:{minute:02d}-{hour:02d}:{minute:02d}(撰寫病歷)"
...
def load_data_page():
    ...
    # 【新增功能 1】顯示記憶的版本資訊
    if st.session_state.remembered_version_info['version_name']:
        st.info(f"💾 記憶中的版本: **{st.session_state.remembered_version_info['version_name']}**")
        st.info(f"🔗 記憶中的連結: {st.session_state.remembered_version_info['main_sheet_url'][:50]}...")
    
    with st.form("load_data_form"):
        version_name = st.text_input(
            "版本識別名稱",
            value=st.session_state.remembered_version_info['version_name'],
        ...
        main_sheet_url = st.text_area(
            ...
            value=st.session_state.remembered_version_info['main_sheet_url'],
        ...
        with col2:
            # 【修改功能 1】改為"載入目前版本班表"
            load_current = st.form_submit_button("🔄 載入目前版本班表")
    
    # 【修改功能 1】處理載入目前版本班表
    if load_current:
        if st.session_state.remembered_version_info['version_name'] and st.session_state.remembered_version_info['main_sheet_url']:
            version_name = st.session_state.remembered_version_info['version_name']
            main_sheet_url = st.session_state.remembered_version_info['main_sheet_url']
            st.info("✅ 已使用記憶中的版本資訊")
            submit_button = True
        else:
            # 如果沒有記憶的版本，使用預設班表
...
def holiday_management_page():
    ...
    # 【新增功能 2】載入記憶的假日設定
    if st.session_state.saved_custom_holidays and not st.session_state.custom_holidays:
        st.session_state.custom_holidays = st.session_state.saved_custom_holidays.copy()
    ...
    if add_holiday:
        ...
        try:
            ...
            # 【新增功能 2】同時更新記憶的假日設定
            st.session_state.saved_custom_holidays[date_key] = f"{reason}({weekday})"
    ...
    if remove_holiday:
        ...
        if date_key in st.session_state.custom_holidays:
            ...
            # 【新增功能 2】同時從記憶的假日設定中移除
            if date_key in st.session_state.saved_custom_holidays:
                st.session_state.saved_custom_holidays.pop(date_key)
    ...
    with col_clear:
        ...
        if st.button("🗑️ 清除所有假日", type="secondary"):
            if st.session_state.custom_holidays:
                ...
                st.session_state.saved_custom_holidays.clear()
                st.success("✅ 已清除所有自定義假日（包含記憶）")
            else:
