...
def query_page():
    ...
    if submit_query:
        ...
        # Excel 匯出按鈕
        if st.button("📊 匯出Excel報表", type="secondary"):
...
def calendar_selector_page():
    """月曆選擇器頁面"""
    st.header("📊 互動式月曆假日選擇器")
    ...
    if 'selected_calendar_holidays' not in st.session_state:
        st.session_state.selected_calendar_holidays = set()
    
    # 年月選擇
    col1, col2, col3 = st.columns([2, 2, 2])
    with col1:
        calendar_year = st.selectbox("年份", 
                                   range(2020, 2031), 
                                   index=st.session_state.calendar_year-2020)
    with col2:
        calendar_month = st.selectbox("月份", 
                                    range(1, 13), 
                                    index=st.session_state.calendar_month-1,
                                    format_func=lambda x: f"{x}月")
    with col3:
        if st.button("🔄 更新月曆"):
            st.session_state.calendar_year = calendar_year
            st.session_state.calendar_month = calendar_month
            st.rerun()
    ...
    st.subheader(f"📅 {calendar_year}年 {month_name} ({calendar_month}月)")
    st.info("💡 點擊下方按鈕選擇自定義假日 (只能選擇平日)")
        ...
        for i, day in enumerate(week):
            with cols[i]:
                if day == 0:
                    ...
                    # 判斷是否為平日
                    current_date = date(calendar_year, calendar_month, day)
                    weekday = current_date.weekday()
                    is_weekend = weekday >= 5
                    
                    date_key = f"{calendar_year}-{calendar_month:02d}-{day:02d}"
                    is_selected = date_key in st.session_state.selected_calendar_holidays
                    
                    if is_weekend:
                        st.markdown(f"<div style='background-color: #f0f0f0; padding: 5px; text-align: center; border-radius: 5px;'>{day}<br><small>(週末)</small></div>", unsafe_allow_html=True)
                    else:
                        button_type = "primary" if not is_selected else "secondary"
                        button_text = f"{day}"
                        
                        if st.button(button_text, key=f"day_{day}", type=button_type):
                            if is_selected:
                                st.session_state.selected_calendar_holidays.remove(date_key)
                            else:
                                st.session_state.selected_calendar_holidays.add(date_key)
                            st.rerun()
    
    # 顯示已選擇的假日
    if st.session_state.selected_calendar_holidays:
        st.subheader("✅ 已選擇的自定義假日")
        selected_list = sorted(list(st.session_state.selected_calendar_holidays))
        
        holiday_display_data = []
        weekdays = ['一', '二', '三', '四', '五', '六', '日']
        ...
        for date_key in selected_list:
            ...
            holiday_display_data.append({
                '日期': date_key,
                '星期': f"週{weekday}"
            })
        
        df_selected = pd.DataFrame(holiday_display_data)
        ...
        with col1:
            if st.button("📤 匯出程式碼"):
                st.subheader("📋 匯出的假日設定程式碼")
                code_lines = ["custom_holidays = {"]
                
                for date_key in sorted(st.session_state.selected_calendar_holidays):
                    year, month, day = date_key.split('-')
                    current_date = date(int(year), int(month), int(day))
                    weekday = weekdays[current_date.weekday()]
                    code_lines.append(f"    '{date_key}': '自定義假日({weekday})',")
                
                code_lines.append("}")
                
                code_text = "\n".join(code_lines)
                st.code(code_text, language="python")
                st.success(f"✅ 總共匯出 {len(st.session_state.selected_calendar_holidays)} 個自定義假日")
        ...
        with col2:
            if st.button("✅ 套用到系統"):
                ...
                for date_key in st.session_state.selected_calendar_holidays:
                    year, month, day = date_key.split('-')
                    current_date = date(int(year), int(month), int(day))
                    weekday = weekdays[current_date.weekday()]
                    st.session_state.custom_holidays[date_key] = f"自定義假日({weekday})"
                    applied_count += 1
                
                st.success(f"✅ 已套用 {applied_count} 個自定義假日到系統")
        
        with col3:
            if st.button("🗑️ 清除選擇"):
                st.session_state.selected_calendar_holidays.clear()
                st.success("✅ 已清除所有選擇")
                st.rerun()
    else:
        st.info("📅 目前沒有選擇任何假日")
