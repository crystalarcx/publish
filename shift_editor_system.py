...
class SessionStateManager:
    ...
    def get_manual_shift(personnel: str, year: int, month: int, day: int) -> Optional[str]:
        ...
        if key in st.session_state.manual_shifts:
            ...
            return st.session_state.manual_shifts[key].get(date_str)
        return None
        ...
        for day in DateHelper.get_month_date_range(year, month):
            try:
                ...
                preview_data.append({
                    ...
                    'day': day,  # 用於編輯
                    'original_shift': effective_shift,  # 原始班次
                })
        ...
        for week_num, week_data in weeks.items():
            with st.container():
                ...
                for i, day_data in enumerate(week_data):
                    with cols[i]:
                        day = day_data['day']
                        current_shift = day_data['班次'] if day_data['班次'] != '休假' else ''
                        ...
                        # 班次選擇框
                        try:
                            default_index = shift_options.index(current_shift) if current_shift in shift_options else 0
                        except ValueError:
                            default_index = 0
                        ...
                        new_shift = st.selectbox(
                            ...
                            shift_options,
                            index=default_index,
                            key=f"shift_edit_{preview_data.personnel}_{preview_data.year}_{preview_data.month}_{day}",
                            label_visibility="collapsed"
                        )
                        
                        # 即時儲存修改
                        if new_shift != current_shift:
                            SessionStateManager.set_manual_shift(
                                preview_data.personnel, preview_data.year, preview_data.month, day, new_shift
                            )
...
def main():
    ...
    st.caption("v2.2 新增手動編輯班次功能 - 指定人員專用")
