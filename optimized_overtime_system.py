# 互動式員工班表加班時數統計系統 (Streamlit版) - 全面優化版
"""
員工班表加班時數統計系統
====================
...
class DataLoader:
    ...
    def load_data_from_urls(main_sheet_url: str, cache_version: int = 0) -> Tuple[Optional[pd.DataFrame], Optional[Dict], str]:
        ...
        try:
            ...
            # 資料驗證
            personnel_count = DataValidator.count_valid_personnel(df)
            
            return df, shift_dict, f"✅ 資料讀取成功！班表: {df.shape}, 人員: {personnel_count} 人, 班種: {len(shift_dict)} 種"
...
class DataValidator:
    ...
    @staticmethod
    def count_valid_personnel(df: pd.DataFrame) -> int:
        """計算有效的人事號數量"""
        if df is None or df.empty:
            return 0
        
        personnel_numbers = df.iloc[1, :].tolist()
        return sum(1 for num in personnel_numbers if pd.notna(num) and str(num).strip())
                ...
                try:
                    ...
                    if shift_value in shift_dict and pd.notna(value) and shift_value:
                        overtime_data = OvertimeCalculator._calculate_daily_overtime(
                            shift_dict[shift_value], current_date, date_str, day_type, is_weekend
                        )
...
class SchedulePreview:
    ...
    @staticmethod
    def generate_schedule_preview(target_personnel: str, year: int, month: int, matching_columns: List[int]) -> PreviewData:
        """
        生成班表預覽資料
...
class ExcelExporter:
    ...
    def export_to_excel(query_result: QueryResult) -> Tuple[bool, Union[io.BytesIO, str], float, float, float, int]:
        ...
        try:
            ...
            # 收集原始時間字串
            date_time_strings = ExcelExporter._collect_time_strings(
                df, shift_dict, query_result.matching_columns, query_result.year, query_result.month
            )
    ...
    @staticmethod
    def _collect_time_strings(df: pd.DataFrame, shift_dict: Dict, matching_columns: List[int], year: int, month: int) -> Dict[str, List[str]]:
        """收集原始時間字串"""
...
def main():
    ...
    st.caption("v2.0 全面優化版 - 更快速、更穩定、更智能")
...
def render_system_status():
    ...
    if st.session_state.df is not None:
        personnel_count = DataValidator.count_valid_personnel(st.session_state.df)
        shift_count = len(st.session_state.shift_dict)
        
        col1, col2, col3 = st.columns(3)
        with col1:
            st.success(f"👥 {personnel_count} 個人事號")
        with col2:
            st.success(f"🔢 {shift_count} 種班次")
        with col3:
            st.success(f"🏖️ {len(st.session_state.custom_holidays)} 個自定義假日")
    else:
...
def render_quick_holidays():
    """渲染快速添加常見假日"""
    st.subheader("🚀 快速添加常見假日")
    ...
    with col1:
        if st.button("🎊 新年假期 (1/1)", type="secondary"):
            add_holiday_to_session(current_year, 1, 1, "元旦")
    
    with col2:
        if st.button("🧧 春節假期", type="secondary"):
            # 添加春節相關假日（示例）
            for day in range(20, 30):  # 假設春節在1月20-30日間
                try:
                    add_holiday_to_session(current_year, 1, day, "春節假期")
                except:
                    pass
    
    with col3:
        if st.button("🌙 中秋節", type="secondary"):
            add_holiday_to_session(current_year, 9, 29, "中秋節")  # 示例日期
